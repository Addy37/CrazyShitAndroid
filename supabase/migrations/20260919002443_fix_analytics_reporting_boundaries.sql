...
with periods as (
  select
    timezone('America/New_York', now())::date as day_start,
    date_trunc('week', timezone('America/New_York', now()))::date as week_start,
    date_trunc('month', timezone('America/New_York', now()))::date as month_start
),
active as (
  select
